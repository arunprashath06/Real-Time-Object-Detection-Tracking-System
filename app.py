...
# Detect if running on Streamlit Cloud (no display / no GPU)
IS_CLOUD = not torch.cuda.is_available()

# Sidebar Controls
st.sidebar.header("⚙️ Pipeline Configuration")

# Model selection - on cloud only offer nano (fast on CPU)
if IS_CLOUD:
...
# Input Source — hide webcam on cloud
if IS_CLOUD:
    source_options = ["Demo Video (Preloaded)", "Upload Video (MP4/AVI)"]
    st.sidebar.info("☁️ Running on Streamlit Cloud (CPU). Webcam is not available. Use Demo or Upload a video.")
else:
    source_options = ["Demo Video (Preloaded)", "Upload Video (MP4/AVI)", "Live Webcam"]

video_source = st.sidebar.radio("Input Source", source_options)
...
if video_source == "Demo Video (Preloaded)":
    ...
    video_path = 0
    st.sidebar.warning("Webcam requires local deployment. Not available on Streamlit Cloud.")
